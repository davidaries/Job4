...
class home_screen:
    ...
    def login_screen(self):
        ...
        module"""
        for _ in PEd.staffers:
            window = self.create_home_screen('+150')
            self.staff_dict_new.append([window, lm(self.root, '~101', self.home, window)])
        for screen_info in self.staff_dict_new:
            screen_info[1].add_entry_id()
            screen_info[1].add_entry_password()
            screen_info[1].login_button()

    def check_login(self, staffer_id, window):
        """"""
