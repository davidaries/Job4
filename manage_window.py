...
class manage_window:
    ...
    def poll_controller(self):
        ...
        if tasks:
            for task in tasks:
                if self.at_home and task not in self.token_list and task not in self.tokens_completed:
    ...
    def send_data(self, token, raw_data):
        if self.at_home:
            ...
            priority = raw_data[2]
            start_time = raw_data[6]
            self.populate_task_btn(task_id, person_id, task_window_info, token, priority, start_time)
            self.row_current += 1

    def populate_task_btn(self, task_id, person_id, task_window_info, token, priority, start_time):
        ...
        self.add_person_to_tasks(person_id, priority, start_time)
    ...
    def add_person_to_tasks(self, person_id, priority, start_time):
        ...
        label_name.grid(column=0, row=self.row_current, ipady=self.row_padding, sticky='W')
        label_time = Label(self.window, text='00:00', font=self.widget_creator.medium_font)  # will be actual time
        label_time.grid(column=1, row=self.row_current, ipady=self.row_padding)
        label_priority = Label(self.window, text=priority, font=self.widget_creator.medium_font)
        label_priority.grid(column=2, row=self.row_current, ipady=self.row_padding)
